...
def fetch_campaign_data(account_id):
    ...
    data = r.json()
    rows = data.get("data", [])
    if rows and isinstance(rows[0], list) and rows[0][0] == "Campaign name":
        rows = rows[1:]
    return [
        {
            "campaign":    row[0],
            "result":      int(row[1] or 0),
...
def generate_report(account_name, campaigns):
    ...
    prompt = f"""
Ти — асистент з аналізу Meta Ads. Сформуй утренній звіт для Telegram.

Клієнт: {account_name}
Дата: {today}
Дані по кампаніях (за сьогодні):
{data_str}

Правила:
1. Заголовок: "📊 Утренній звіт Meta Ads\nКлієнт: {account_name}\nДата: {today}"
2. По кожній кампанії окремо: Назва | Результат | Витрати | Покази | CTR | CPC | CPM
3. Підсумковий рядок "Разом"
4. Статус кожної кампанії:
   🟢 — все добре
   🟡 — слідкувати
   🔴 — терміново перевірити
5. Коротко, мова українська, plain text для Telegram.
"""
